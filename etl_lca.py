...
- adds helpers (PRIMARY_DATE, years, locations)
- deduplicates by CASE_NUMBER
- saves CSV + Parquet
"""
...
def load_one(path: Path, source_tag: str) -> pd.DataFrame:
    ...
    # Force strings for leading zeros etc.
    TEXT_FORCE = [
    ...
    # Locations
    df["WORKSITE_LOCATION"] = df["WORKSITE_CITY"].astype("string").fillna("") + ", " + df["WORKSITE_STATE"].astype("string").fillna("")
    df["EMPLOYER_LOCATION"] = df["EMPLOYER_CITY"].astype("string").fillna("") + ", " + df["EMPLOYER_STATE"].astype("string").fillna("")
...
def main():
    ...
    print(f"Rows after merge & dedupe: {len(merged):,}")

    merged.to_csv(OUT_CSV, index=False)

    # Make strings consistent for Parquet
